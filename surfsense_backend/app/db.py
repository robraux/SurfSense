...
async def setup_indexes():
    async with engine.begin() as conn:
        ...
        await conn.execute(
            text(
                "CREATE INDEX IF NOT EXISTS document_vector_index ON documents USING hnsw (embedding vector_cosine_ops)"
            )
        )
        await conn.execute(
            text(
        ...
        await conn.execute(
            text(
                "CREATE INDEX IF NOT EXISTS chucks_vector_index ON chunks USING hnsw (embedding vector_cosine_ops)"
            )
        )
        await conn.execute(
            text(
